...
export const listarEstudiantes = async () => {
  try {
    const [rows] = await pool.query(
      "SELECT * FROM estudiantes WHERE status = 1"
    );
    return rows;
  } catch (error) {
    throw { status: 500, message: "Error al obtener estudiantes" };
...
export const eliminarEstudiante = async (id) => {
  try {
    const [result] = await pool.query(
      "UPDATE estudiantes SET status = 0 WHERE id_estudiante = ?",
      [id]
    );
    ...
    throw {
      status: 500,
      message: `Error al eliminar (lógicamente) el estudiante con ID ${id}`,
    };
  }
};
